...
export const adminDeleteHoraireCreneau = async (req, res) => {
  ...
  try {
    ...
    const checkQuery =
      "SELECT COUNT(*) as count FROM RESERVATION WHERE HORAIRES_ID = ?";
...
export const adminGetFormDataHorairesCreneaux = async (req, res) => {
  ...
  try {
    ...
    const periodesQuery =
      "SELECT PERIODE_ID, LIBELLE_PERIODE FROM PERIODE_CRENEAU ORDER BY LIBELLE_PERIODE ASC";
    ...
    console.error(
      "Erreur dans adminGetFormDataHorairesCreneaux :",
      error.message
    );
    return res.status(500).json({
      success: false,
...
export const adminUpdateHoraireCreneau = async (req, res) => {
  ...
  try {
    ...
    const params = [
      ...
      data.direction_id,
      data.horaire_depart,
      data.horaire_passage1 === undefined || data.horaire_passage1 === ""
        ? null
        : data.horaire_passage1,
      data.horaire_passage2 === undefined || data.horaire_passage2 === ""
        ? null
        : data.horaire_passage2,
      data.horaire_passage3 === undefined || data.horaire_passage3 === ""
        ? null
        : data.horaire_passage3,
      data.horaires_id,
    ];
...
export const getPendingReservations = async (req, res) => {
  try {
    ...
    const formattedReservations = reservations.map((r) => ({
      ...
      user_id: r.USER_ID,
      user_nom: r.user_nom,
      reservation_date: new Date(r.DATE_RESERVATION)
        .toISOString()
        .split("T")[0],
...
export const updatePontStatus = async (req, res) => {
  try {
    ...
    // SI AUCUN PONT_ID N'EST FOURNI, ON RENVOIE LA LISTE DES PONTS DISPONIBLES (PAS DE VÉRIF TOKEN)
    if (!pontId) {
    ...
    // À PARTIR D'ICI, ON EXIGE LE TOKEN ET L'ADMIN
    if (!req.user || req.user.type_user_id !== 1) {
      return res.status(403).json({
        success: false,
        message: "Accès réservé aux administrateurs",
    ...
    if (!allowedStatus.includes(status)) {
      return res.status(400).json({
        success: false,
        message: "Statut invalide. Valeurs possibles : ouvert, ferme, stop.",
      });
    }
